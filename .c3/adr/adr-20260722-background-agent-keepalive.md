...
goal: |-
    ...
    Agent (Task tool) run is still in flight. Arm the keep-alive guard from the
    SDK's background_tasks_changed level signal (primary), an Agent-launch
    tool_result regex (PTY + version-skew fallback), and bump lastUsedAt on
    stream activity so self-wake turns never count as idle.
status: accepted
date: "2026-07-22"
---
