---
id: c3-227
c3-seal: a5740a595ff78c8f3117bf0a052482dc7da217bbc69163c2c75a35c259d89e9c
title: auto-continue
type: component
parent: c3-2
goal: |-
...
| Cron run-event retention | IN | The log itself never expires, so its only bound on cron run events is c3-233's compactCronRunEvents, applied at applyAutoContinueToState (shared by live append and boot replay) and at snapshot load — the only two places the per-chat array is built. Job lifecycle events and every non-cron event are never dropped | c3-233 | src/server/cron/compact.ts, src/server/event-store-chat-lifecycle.ts, src/server/event-store-snapshot.ts |
...
| src/server/auto-continue/limit-detector.ts | Contract and Foundational Flow (rate-limit classifier) | Pattern detail | src/server/auto-continue/limit-detector.ts |
| src/server/auto-continue/auth-error-detector.ts | Contract and Foundational Flow (auth-error classifier) | Pattern detail | src/server/auto-continue/auth-error-detector.ts |
...
| src/server/auto-continue/e2e.test.ts | Contract and Business Flow (primary path) | Test framing | src/server/auto-continue/e2e.test.ts |
