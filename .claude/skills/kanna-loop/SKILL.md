...
**Arm-time chunk audit (`KANNA_SYSTEM_ONE`).** Beside the oracle audit,
...
on the created task's JSON). Opt-in (`KANNA_SYSTEM_ONE=enabled` plus
`TYPESAFE_API_KEY`, the same switch that registers the `decide` tool),
advisory, fail-open — see the "TypeSafe System One" section in CLAUDE.md for
the thresholds and why the question phrasing is load-bearing.
