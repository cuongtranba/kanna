...
- **Phase pipeline (F4):** Each task runs through an ordered list of `OrchPhaseSpec` entries (implement → review × 2 → fix). Each phase spawns a fresh worker via the injected `StartWorker`. `{{TASK}}`, `{{DIFF}}`, `{{PRIOR}}` template vars carry context forward across phases. `{{DIFF}}` is bounded by `boundDiff` (`orchestration-diff.ts`, 64k char budget: banner with per-file +/- counts, order-preserving greedy packing so a giant lockfile can't starve later source files, `[omitted]` markers, full diff reachable via `git diff` in the worker's worktree cwd). Review-kind phase outputs are combined by `combineReviewOutputs` (`orchestration-review.ts`): reviewers are prompted for a fenced-JSON `OrchReviewFinding[]` (`{file, line, problem, suggestedFix, severity}` — or `NO_FINDINGS`); conformant replies are merged, deduped across the parallel reviewers (same file+line, or same file + normalized problem when lines are null; higher severity survives), and rendered as a compact numbered `{{PRIOR}}` block for the fix phase. ANY unparsed reply falls back to the raw `---` join — format drift never fails a run or loses reviewer signal. `contextPrompt` is prepended to every worker prompt; `scopePaths` is injected only into implement-kind phases.
