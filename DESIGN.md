...
- **One ambient loop is sanctioned, and it is a heartbeat, not a pulse.** The `Background tasks` header beats its `Activity` glyph (`--animate-kanna-heartbeat`, `motion-safe:` only) because that panel exists *only* while work is running off-turn — the beat says the process is alive when nothing else in the row moves. It is a lub-dub with a full second of rest, not a sinusoid: the rest is what separates it from the throbbing dot the Don'ts ban. Scale only, so the header never reflows. The glyph takes `text-warning-text` — the ink form, never raw `text-warning` — so it reads as the same amber as the `running` dot beside it rather than as a second, unrelated signal. This does not license a second one; a status that a label already carries stays still.
...
- **A tracked finger gets no easing.** While a gesture is in progress the surface follows the finger 1:1; easing describes what happens *after* release, and applying it mid-gesture makes the surface lag the thumb holding it. The mobile drawer is the case: `sidebarDragProgress` draws it, `evaluateSidebarSwipe` still decides the outcome, and the thresholds a user has already learned are never changed by the fact that the gesture now has frames.
- **Anything appended outside React removes itself on every path.** The new-session title carry clones a title into `document.body` to fly it; a clone that outlives its animation is duplicated text floating over the app with nothing to dismiss it. Bound the life with a timeout that does not depend on the animation completing, and never build a selector by interpolating a runtime value.
