...
- **`PlateCaption.fact` is required.** "Every label states a fact" survives as a compile error and not as a convention. `SectionCaption.fact` is optional by contrast: a single-value panel has nothing countable, and inventing a number to satisfy the rule is worse than omitting it.
...
- **The speaker is a margin gloss**, `aria-hidden` — the transcript already exposes authorship structurally, so announcing it before every entry is screen-reader noise. On a narrow measure it stacks above the text rather than holding a fixed gutter.
