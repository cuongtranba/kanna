...
ws-router.ts (1703 LOC): extract the `project.*`, `sessions.*`, `sidebar.reorderProjectGroups`, `system.*`, and `update.*` command handlers into `src/server/ws-router-project.ts`. Handlers to move: `system.ping` (ack only), `update.check/install/reload` (3 handlers, UpdateManager dep), `project.open/create/remove/setStar/readDiffPatch` (5 handlers, store + ensureProjectDirectory + resolveLocalPath + refreshDiscovery + resolvedDiffStore + resolvedAnalytics), `sessions.importClaude` (importClaudeSessions + refreshDiscovery), `sidebar.reorderProjectGroups` (store.setSidebarProjectOrder), `system.openExternal` (openExternal). Define a `ProjectCommandDeps` interface: `store` (getProject, openProject, removeProject, setProjectStar, setSidebarProjectOrder, state.projectIdsByPath), `updateManager` (optional: checkForUpdates, installUpdate, forceReload), `diffStore` (readPatch), `analytics` (track), `refreshDiscovery: () => Promise<unknown>`, `ensureProjectDirectory: (path: string) => Promise<void>`, `resolveLocalPath: (path: string) => string`, `importClaudeSessions: (deps: { store: ... }) => Promise<ImportResult>`, `openExternal: (cmd: ...) => Promise<void>`, `terminals` (closeByCwd), `send`, `broadcastSidebar: () => Promise<void>`. Create `handleProjectCommand(deps, command, id): Promise<boolean>`. Add `ws-router-project.test.ts` with at least 8 tests covering at least one handler from each group (system, update, project, sessions, sidebar). Wire the fall-through in ws-router.ts. Verify targeted lint/typecheck/test, commit, push, update this file.
