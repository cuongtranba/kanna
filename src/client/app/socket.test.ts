...
describe("KannaSocket", () => {
  ...
  test("subscribe delivers bg-tasks snapshot to listener", () => {
    const socket = new KannaSocket("ws://localhost/ws")
    socket.start()
    const ws = FakeWebSocket.instances[0]!
    ws.open()
    ...
    const unsubscribe = socket.subscribe<unknown>(
      { type: "bg-tasks" },
      (snapshot) => snapshots.push(snapshot)
    )

    const subMsg = ws.sent.find((m) => m.type === "subscribe" && (m.topic as Record<string, unknown>)?.type === "bg-tasks")
    expect(subMsg).toBeDefined()

    const task = { kind: "draining_stream", id: "t1", chatId: "c1", startedAt: 0, lastOutput: "" }
    ws.receive({ v: 1, type: "snapshot", id: subMsg?.id, snapshot: { type: "bg-tasks", data: [task] } })

    expect(snapshots).toHaveLength(1)
    expect(snapshots[0]).toEqual([task])

    unsubscribe()
    socket.dispose()
  })

  test("subscribe delivers bg-tasks diff events to eventListener", () => {
    const socket = new KannaSocket("ws://localhost/ws")
    socket.start()
    const ws = FakeWebSocket.instances[0]!
    ws.open()
    ...
    const unsubscribe = socket.subscribe<unknown[], unknown>(
      { type: "bg-tasks" },
      () => { /* snapshot ignored */ },
    ...
    const task = { kind: "draining_stream", id: "t2", chatId: "c1", startedAt: 0, lastOutput: "" }
    ws.receive({ v: 1, type: "event", id: subMsg?.id, event: { type: "bg-tasks.added", task } })
    ws.receive({ v: 1, type: "event", id: subMsg?.id, event: { type: "bg-tasks.updated", task: { ...task, lastOutput: "hi" } } })
    ws.receive({ v: 1, type: "event", id: subMsg?.id, event: { type: "bg-tasks.removed", task } })

    expect(events).toHaveLength(3)
    expect((events[0] as Record<string, unknown>).type).toBe("bg-tasks.added")
    expect((events[1] as Record<string, unknown>).type).toBe("bg-tasks.updated")
    expect((events[2] as Record<string, unknown>).type).toBe("bg-tasks.removed")
