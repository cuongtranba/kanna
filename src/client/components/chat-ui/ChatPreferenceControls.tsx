  ...
  if (disabled) {
    return (
      <button
        disabled
        className={cn(
          "flex min-h-[36px] items-center gap-1.5 px-2 py-1 text-sm rounded-md text-muted-foreground [&>svg]:shrink-0 opacity-70 cursor-default [&>span]:whitespace-nowrap",
          triggerClassName
        )}
      >
        {trigger}
    ...
    <div className={cn("flex md:justify-center items-center gap-0.5", className)}>
      ...
      {selectedProvider === "codex" ? (
        <InputPopover
          ...
          triggerClassName={codexModelOptions?.fastMode ? "text-success-text" : undefined}
        >
          {(close) => (
            <>
              <PopoverMenuItem
      ...
      {showPlanMode ? (
        <InputPopover
          ...
          triggerClassName={planMode ? "text-info-text" : undefined}
        >
          {(close) => (
            <>
              <PopoverMenuItem
