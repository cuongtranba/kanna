...
const DESCRIPTIONS: Record<string, { default: string; description: string }> = {
  ...
  KANNA_SYSTEM_ONE: { default: '(unset)', description: 'Set to "enabled" (with TYPESAFE_API_KEY also set) to turn on the features backed by TypeSafe\'s System One model: the `decide` tool, which lets the agent ask calibrated yes/no, choice and score questions over evidence it supplies, and the chunk gate, which has setup_loop and task_create append a "Chunk audit" note naming planned tasks that read as status lines, only point at another file, are too vague for a fresh-context worker, or bundle several units of work. Advisory only: the gate never blocks arming or creating a task and fails open when the API is unreachable. Text sent to the model leaves the machine for api.typesafe.ai.' },
