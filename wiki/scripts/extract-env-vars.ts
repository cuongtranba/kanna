...
const DESCRIPTIONS: Record<string, { default: string; description: string }> = {
  ...
  KANNA_IMPORT_FOLLOW_MAX_FAILURES: { default: '3', description: 'Consecutive failed live-tail deltas before the registry stops following a session and drops its live badge. Every failure kind is permanent for a given file (over the size cap, unreadable, chat row gone), and a file that keeps growing never reaches the idle stop, so this is what bounds a permanently-failing source.' },
