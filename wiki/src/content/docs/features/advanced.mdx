---
title: Advanced
description: Custom MCP servers, workflow panel, agent chat re-entry, local skills, token cost, self-update, expose_port, mermaid, transcript export, keybindings, password gate, PWA.
---
...
:::note[Removed]
The timer-based `mcp__kanna__schedule_wakeup` tool and its `KANNA_MAX_AGENT_WAKES` cap no longer exist — they were removed when re-entry became notification-driven. A self-polling agent lost momentum across context compaction, which is what the background-delegation path above fixes. Claude Code's own native `ScheduleWakeup` stays disallowed under the PTY driver for the same reason.
:::
