...
export const envVars: EnvVar[] = [
  { name: 'KANNA_BACKGROUND_TASK_GUARD', default: "enabled", description: "Set to \"disabled\" to stop Kanna asking the model to retrieve a background task it left running when a turn ended. The ask happens at most once per task, and only on a session whose task list comes from the SDK — the one case where the deadline-based wake ladder never runs." },
